...
public class ConcurrentPrinter : IPrinter
{
    private readonly ConcurrentQueue<FileAndTaskSource> _printingQueue = [];
    private readonly ConcurrentPeriodicWorker _periodicWorker;
    private readonly int _printingTime;
    private readonly object _lock = new();

    public ConcurrentPrinter(int printingTimeInMs, int checkingTimeInMs)
    {
    ...
    private async Task TryToPrint(CancellationToken token)
    {
        await Task.Delay(_printingTime, token);
